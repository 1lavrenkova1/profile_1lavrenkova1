...
int main(int argc, char** argv) { 
    ...
    key_t key; //будем использовать 1 и тот же ключ для семафора и для shm  
    ...
    if((buf = (int*)shmat(shmemory, 0, 0)) < 0) {     
        ...
        exit(1);   
    } // устанавливаемобработчиксигнала  
    signal(SIGINT, intHandler); //создаем группу из 3 семафоров  //1 - число свободных ячеек  //2 - число занятых ячеек// 3 работа с памятью  
    if((semaphore = semget(key, 3, IPC_CREAT | 0666)) < 0) {     
        printf("Error while creating semaphore\n");     
        kill(getpid(),SIGINT);   
    } // устанавливаем индекс в -1,  //первый записывающий клиентустановит его в ноль  
    buf[BUF_SIZE] = -1; // инициализируем массив -1  
    int j = 0;   
    ...
    int i = 0;   
    for(i = 0; i < 15; ++i) { // ждем, пока будет хоть одна непустая ячейка
        ...
        printf("Remove %d from cell %d\n", res,buf[BUF_SIZE]+1);     // освобождаемпамять     
        if(semop(semaphore, mem_unlock, 1) < 0) {       
            printf("execution complete\n");       
            kill(getpid(),SIGINT);     
        }       // увеличиваемчислопустыхячеек     
