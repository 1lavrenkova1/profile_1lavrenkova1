...
int main(int argc, char** argv) {   
    ...
    key_t key; 
    int shmemory; 
    int semaphore; //будем использовать 1 и тот же ключ для семафора и для shm  
    ...
    char tb[10];   
    int i = 0; 
    for(i = 0; i < 10;++i) { //ждем, пока будет хоть одна свободная ячейка  
        ...
        ++buf[BUF_SIZE];   
        buf[buf[BUF_SIZE]] = send++;   //освобождаемдоступкпамяти  
        if(semop(semaphore, mem_unlock, 1) < 0) {     
            printf("Can't execute a operation\n");     
            exit(1);   
        }    //увеличиваемчислозанятыхячеек  
