...
bool check_new_square(int x, int y, std::vector<Square>& table){
    for (size_t i = 0; i < table.size(); i++){
        if ((table[i].x_coord <= x && x < table[i].x_coord + table[i].size) && (table[i].y_coord <= y && y < table[i].y_coord + table[i].size)){
            //std::cout<<"1"<<table[i].x_coord<<table[i].y_coord <<" "<<x<<y<<table[i].y_coord + table[i].size<<"\n";
            return true;
        }
    }
    //std::cout<<"2"<<x<<y<<"\n";
...
int main(){
    for (int i = 2; i < 41; i++){
        ...
        std::cout<< std::chrono::duration_cast<std::chrono::microseconds>(time).count() - std::chrono::duration_cast<std::chrono::milliseconds>(time).count() * 1000 /*<< "mcs\n"*/;
        std::cout<<"\n";
    }
    return 0;
}
